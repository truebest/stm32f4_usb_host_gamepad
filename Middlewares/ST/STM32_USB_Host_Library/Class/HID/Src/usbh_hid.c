...
static USBH_StatusTypeDef USBH_HID_InterfaceInit(USBH_HandleTypeDef *phost)
{
  ...
  interface = USBH_FindInterface(phost, phost->pActiveClass->ClassCode, HID_NO_SUB_CODE, 0xFFU);
  ...
  else if (phost->device.CfgDesc.Itf_Desc[interface].bInterfaceProtocol  == HID_NO_SUB_CODE)
  {
...
HID_TypeTypeDef USBH_HID_GetDeviceType(USBH_HandleTypeDef *phost)
{
  ...
  if (phost->gState == HOST_CLASS)
  {
    ...
    else if (InterfaceProtocol == HID_NO_SUB_CODE)
    {
        type = HID_GAMEPAD;
    }
  }
